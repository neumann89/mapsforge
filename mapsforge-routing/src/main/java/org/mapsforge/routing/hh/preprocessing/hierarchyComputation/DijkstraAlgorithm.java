 ...
 */
package org.mapsforge.routing.hh.preprocessing.hierarchyComputation;

import gnu.trove.map.hash.TIntObjectHashMap;

import java.util.LinkedList;

import org.mapsforge.routing.preprocessing.data.BinaryMinHeap;
import org.mapsforge.routing.preprocessing.data.IBinaryHeapItem;
import org.mapsforge.routing.hh.server.HHStaticGraph;
import org.mapsforge.routing.hh.server.HHStaticGraph.HHStaticEdge;
import org.mapsforge.routing.hh.server.HHStaticGraph.HHStaticVertex;
...
public final class DijkstraAlgorithm {
	 ...
	 */
	public static int shortestDistance(HHDynamicGraph.HHDynamicVertex source, HHDynamicGraph.HHDynamicVertex target,
		...
		while (!queue.isEmpty()) {
			...
			// relax forward edges
			for (HHDynamicGraph.HHDynamicEdge e : u.vertex.getOutboundEdges(lvl)) {
	 ...
	 * @return the distance.
	 */
	public static int shortestDistance(HHDynamicGraph.HHDynamicVertex source, int rank, boolean forward,
	 ...
	 */
	public static LinkedList<HHDynamicGraph.HHDynamicEdge> shortestPathEdges(HHDynamicGraph.HHDynamicVertex source,
			HHDynamicGraph.HHDynamicVertex target, boolean forward, boolean backward, int lvl) {
		DijkstraTreeVertex currentVertex = shortestPath(source, target, forward, backward, lvl);
		// get list of edges from source to target
		LinkedList<HHDynamicGraph.HHDynamicEdge> list = null;
		if (currentVertex != null) {
			list = new LinkedList<HHDynamicGraph.HHDynamicEdge>();
	 ...
	 */
	public static LinkedList<HHDynamicGraph.HHDynamicVertex> shortestPathVertices(HHDynamicGraph.HHDynamicVertex source,
			HHDynamicGraph.HHDynamicVertex target, boolean forward, boolean backward, int lvl) {
		DijkstraTreeVertex currentVertex = shortestPath(source, target, forward, backward, lvl);
		// get list of edges from source to target
		LinkedList<HHDynamicGraph.HHDynamicVertex> list = null;
		if (currentVertex != null) {
			list = new LinkedList<HHDynamicGraph.HHDynamicVertex>();
	 ...
	 */
	public static LinkedList<DijkstraTreeVertex> shortestPathTree(HHDynamicGraph.HHDynamicVertex source,
			boolean forward, boolean backward, int lvl) {
	 ...
	 */
	public static LinkedList<Integer> shortestPathHopIndices(HHDynamicGraph.HHDynamicVertex source,
			HHDynamicGraph.HHDynamicVertex target, boolean forward, boolean backward, int lvl) {
		DijkstraTreeVertex currentVertex = shortestPath(source, target, forward, backward, lvl);
		// get list of edges from source to target
	...
	static LinkedList<HHDynamicGraph.HHDynamicEdge> selectHighwayEdges(HHDynamicGraph.HHDynamicVertex source,
			boolean forward, boolean backward, int lvl) {
		LinkedList<HHDynamicGraph.HHDynamicEdge> selectedEdges = new LinkedList<HHDynamicGraph.HHDynamicEdge>();
		if (!forward && !backward) {
	...
	private static DijkstraTreeVertex shortestPath(HHDynamicGraph.HHDynamicVertex source,
			HHDynamicGraph.HHDynamicVertex target, boolean forward, boolean backward, int lvl) {
		BinaryMinHeap<DijkstraTreeVertex, Integer> queue = new TreeVertexHeap(INITIAL_HEAP_SIZE);
		TIntObjectHashMap<DijkstraTreeVertex> discoveredVertices = getMap();
		DijkstraTreeVertex s = new DijkstraTreeVertex(source, 0, null, null, -1);
	...
	private static LinkedList<DijkstraSlackVertex> dag(HHDynamicGraph.HHDynamicVertex source, boolean forward,
			boolean backward, int lvl) {
	...
	private static class DijkstraSlackVertex extends DijkstraDistanceVertex {
		...
		public DijkstraSlackVertex(HHDynamicGraph.HHDynamicVertex vertex, int distance) {
	...
	private static class ParentEntry {
		...
		public final HHDynamicGraph.HHDynamicEdge edgeToParent;

		public ParentEntry(DijkstraSlackVertex vertex, HHDynamicGraph.HHDynamicEdge edge) {
			this.parent = vertex;
	...
	static class DijkstraTreeVertex extends DijkstraDistanceVertex {
		...
		public HHDynamicGraph.HHDynamicEdge edgeToParent;
		public int hopIdx;

		public DijkstraTreeVertex(HHDynamicGraph.HHDynamicVertex vertex, int distance,
				DijkstraTreeVertex parent, HHDynamicGraph.HHDynamicEdge edgeToParent, int hopIdx) {
			super(vertex, distance);
			this.parent = parent;
	...
	private static class DijkstraDistanceVertex implements IBinaryHeapItem<Integer> {
		...
		public HHDynamicGraph.HHDynamicVertex vertex;
		public int distance;
		public int heapIndex;

		public DijkstraDistanceVertex(HHDynamicGraph.HHDynamicVertex vertex, int distance) {
			this.vertex = vertex;
			this.distance = distance;
		}
