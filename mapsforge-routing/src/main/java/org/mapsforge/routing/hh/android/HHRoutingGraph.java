...
import org.mapsforge.routing.android.data.Cache;
...
import org.mapsforge.routing.WGS84;
import org.mapsforge.routing.hh.preprocessing.mobile.HHGlobals;
 ...
 */
class HHRoutingGraph implements Closeable {
	...
	public HHRoutingGraph(File hhBinaryFile, int cacheSizeBytes) throws IOException {
		...
		this.blockIndex = new StaticRTree(hhBinaryFile, startAddrRTree, endAddrRTree,
				HHGlobals.STATIC_RTREE_HEADER_MAGIC);
	...
	public HHVertex getNearestVertex(GeoCoordinate c, double maxDistanceMeters)
		...
		double alphaLon = (maxDistanceMeters * 180)
				/ (Math.cos(Math.toRadians(c.getLatitude())) * WGS84.EQUATORIAL_RADIUS * Math.PI);
		double alphaLat = (maxDistanceMeters * 180) / (WGS84.EQUATORIAL_RADIUS * Math.PI);
