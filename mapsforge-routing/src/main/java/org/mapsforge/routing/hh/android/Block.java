 ...
 */
package org.mapsforge.routing.hh.android;
...
final class Block implements CacheItem {
	...
	/**
	 * Number of vertices of typeA. These vertices have neighborhood and also belong to the next level.
	 */
	private final short numVerticesTypeA;
	/**
	 * Number of vertices of typeB. These vertices have neighborhood but do not belong to the next
	 * level.
	 */
	private final short numVerticesTypeB;
	/**
	 * number of vertices if tyoeC. The vertices have their neighborhood set to infinity and thus do not
	 * belong to the next level.
	 */
	private final short numVerticesTypeC;
	/**
	 * the minimum latitude in micro degrees of all coordinates stored within this block, way point
	 * coordinate and vertex coordinates.
	 */
	private final int minLatitudeE6;
	/**
	 * the minimum longitude in micro degrees of all coordinates stored within this block, way point
	 * coordinate and vertex coordinates.
	 */
			...
			if (level == 0 && edge.isForward) {
				...
				if (hasName) {
					int byteOffset = Deserializer.readUInt(data, bitsPerStreetNameOffset,
							offset);
					edge.name = IOUtils.getZeroTerminatedString(data, offsetStreetNames + byteOffset);
				}
				...
				if (hasRef) {
					int byteOffset = Deserializer.readUInt(data, bitsPerStreetNameOffset,
							offset);
					edge.ref = IOUtils.getZeroTerminatedString(data, offsetStreetNames + byteOffset);
				}
