...
import org.mapsforge.routing.graph.IRgDAO;
import org.mapsforge.routing.graph.IRgEdge;
import org.mapsforge.routing.graph.IRgVertex;
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.HHDynamicGraph.HHDynamicEdge;
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.HHDynamicGraph.HHDynamicVertex;
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.HHGraphProperties.HHLevelStats;
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.util.arrays.BitArraySynchronized;
import org.mapsforge.routing.hh.server.DistanceTable;
...
/**
 *
 * This class is responsible for the complete process of preprocessing. As input, it reads a properties
 * file res/hhPreprocessing.properties. tuning params for preprocessing: (names similar to highway
 * hierarchies paper)
 *
 ...
 *
 * 5. downgrade edge = downgrade edges leaving core (check of restriction 2 is no more needed during
 * query)
 */
public final class HHComputation {
	...
	public static <V extends IRgVertex, E extends IRgEdge> void doPreprocessing(
			IRgDAO<V, E> rgDao, int h, int hopLimit, double c,
		...
		HHDynamicGraph graph = HHDynamicGraph.importRoutingGraph(rgDao);
	...
	private static int[] regroupVerticesByCoreLevel(HHDynamicGraph graph) {
		...
		return originalIds;
		/*
		 * int lvlOffset = 0; for (int lvl = -1; lvl <= graph.numLevels(); lvl++) { int i = lvlOffset;
		 * while (i < graph.numVertices(0) && getCoreLevel(graph.getVertex(i)) == lvl) i++;
		 *
		 * for (int j = graph.numVertices(0) - 1; j > i; j--) { if (getCoreLevel(graph.getVertex(j)) ==
		 * lvl) { graph.swapVertexIds(graph.getVertex(i), graph.getVertex(j)); int tmp = originalIds[i];
		 * originalIds[i] = originalIds[j]; originalIds[j] = tmp; while (i < graph.numVertices(0) &&
		 * getCoreLevel(graph.getVertex(i)) == lvl) i++; } } lvlOffset = i; } int[] orgIdToId = new
		 * int[graph.numVertices(0)]; for (int i = 0; i < orgIdToId.length; i++) {
