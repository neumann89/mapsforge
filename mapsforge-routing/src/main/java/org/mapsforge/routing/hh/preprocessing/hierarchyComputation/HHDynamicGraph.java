...
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.util.arrays.BitArrayGrowable;
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.util.arrays.IntArrayGrowable;
...
class HHDynamicGraph implements Serializable {
	...
	public static <V extends IRgVertex, E extends IRgEdge> HHDynamicGraph importRoutingGraph(
		...
		for (Iterator<? extends E> iter = dao.getEdges().iterator(); iter.hasNext();) {
			...
			E e = iter.next();
			// System.out.println(e.toString());wFunc.getWeightInt(e)
			if (e == null)
				System.out.println("[HHDYNAMICGRAPH -]Edge is Empty!!!!!!!!!");
			graph.addEdge(e.getSourceId(), e.getTargetId(), e.getWeight(), true, e
					.isUndirected(), lvl);
			graph.addEdge(e.getTargetId(), e.getSourceId(), e.getWeight(), e
