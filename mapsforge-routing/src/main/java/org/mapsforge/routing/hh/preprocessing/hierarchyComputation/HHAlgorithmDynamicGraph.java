...
class HHAlgorithmDynamicGraph {
	...
	public int shortestDistance(HHDynamicGraph.HHDynamicVertex source, HHDynamicGraph.HHDynamicVertex target) {
		int numSettled = 0;
		LinkedList<HHDynamicGraph.HHDynamicEdge> list = new LinkedList<HHDynamicGraph.HHDynamicEdge>();
	...
	public boolean relaxAdjacentEdges(DiscoveredVertex u, int direction, int lvl, int gap) {
		...
		for (HHDynamicGraph.HHDynamicEdge e : u.vertex.getOutboundEdges(lvl)) {
			...
			HHDynamicGraph.HHDynamicVertex _v = e.getTarget();
	...
	private class DiscoveredVertex implements IBinaryHeapItem<HeapKey> {
		HHDynamicGraph.HHDynamicVertex vertex;
		HHDynamicGraph.HHDynamicEdge edgeToParent;
		HeapKey key;
		int heapIdx;

		public DiscoveredVertex(HHDynamicGraph.HHDynamicVertex vertex, HHDynamicGraph.HHDynamicEdge edgeToParent,
				HeapKey key) {
			this.vertex = vertex;
	...
	public int shortestDistanceDijkstra(HHDynamicGraph.HHDynamicVertex source, HHDynamicGraph.HHDynamicVertex target) {
		int numSettled = 0;
		...
		while (!queue[FWD].isEmpty() || !queue[BWD].isEmpty()) {
			...
			boolean forward = direction == FWD;
			for (HHDynamicGraph.HHDynamicEdge e : u.vertex.getOutboundEdges(0)) {
				if (forward && !e.isForward()) {
					continue;
				}
				if (!forward && !e.isBackward()) {
	...
	public int dijkstra(HHDynamicGraph.HHDynamicVertex source, HHDynamicGraph.HHDynamicVertex target, int lvl) {
		int numSettled = 0;
	...
	public int dijkstraBackwardTestInbound(HHDynamicGraph.HHDynamicVertex source, HHDynamicGraph.HHDynamicVertex target) {
		int numSettled = 0;
		...
		while (!queue[FWD].isEmpty()) {
			...
			for (HHDynamicGraph.HHDynamicEdge e : u.vertex.getInboundEdges(0)) {
				if (!e.isBackward()) {
					continue;
				}
