...
import org.mapsforge.routing.hh.preprocessing.hierarchyComputation.util.renderer.RendererV2;
...
public class HHRouter implements Router {
	...
	@Override
	public Edge[] getNearestEdges(GeoCoordinate coordinate) {
		Vertex vertex = getNearestVertex(coordinate);
		if (vertex == null) {
			return null;
		}
	...
	public Vertex getNearestVertex(GeoCoordinate coordinate) {
		try {
			HHVertex vertex = routingGraph.getNearestVertex(coordinate, MAX_RTREE_SEARCH_RADIUS);
			if (vertex == null) {
				return null;
			}
	...
	@Override
	public Iterator<? extends Vertex> getVerticesWithinBox(Rect boundingBox) {
		try {
			final LinkedList<HHVertex> vertices = routingGraph.getVerticesWithinBBox(boundingBox);
