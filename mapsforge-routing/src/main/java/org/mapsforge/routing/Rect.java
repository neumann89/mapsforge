...
public class Rect {
	...
	/**
	 * Constructs a rectangle defined by the coordinate as its center and the distance as the distance
	 * from the center to the border.
	 * 
	 * @param gc
	 *            The coordinate as its center.
	 * @param distanceMeters
	 *            The distance from its center to its border.
	 */
	public Rect(GeoCoordinate gc, int distanceMeters) {
		double alphaLongitude = GeoCoordinate.longitudeDistance(distanceMeters, gc.getLatitude());
		double alphaLatitude = GeoCoordinate.latitudeDistance(distanceMeters);

		this.minLongitudeE6 = GeoCoordinate.doubleToInt(gc.getLongitude() - alphaLongitude);
		this.maxLongitudeE6 = GeoCoordinate.doubleToInt(gc.getLongitude() + alphaLongitude);
		this.minLatitudeE6 = GeoCoordinate.doubleToInt(gc.getLatitude() - alphaLatitude);
		this.maxLatitudeE6 = GeoCoordinate.doubleToInt(gc.getLatitude() + alphaLatitude);
		validate();
	}
	...
	/**
	 * Creates a new rectangle from a String containing comma-separated coordinates in the order minLat,
	...
	public static Rect fromString(String rectString) {
		...
		GeoCoordinate upperLeft = GeoCoordinate.fromString(parts[2] + "," + parts[1]);
		GeoCoordinate bottomRight = GeoCoordinate.fromString(parts[0] + "," + parts[3]);
	...
	public GeoCoordinate center() {
        return new GeoCoordinate(
                (this.minLatitudeE6 + this.maxLatitudeE6) / 2,
                (this.minLongitudeE6 + this.maxLongitudeE6) / 2
        );
	}

	/**
	 * Computes the distance between this and another rectangle. The distance between overlapping
	 * rectangles is 0. Otherwise, the distance is the spherical distance between the closest points.
	 * 
	 * @param otherRect
	...
	/**
	 * Enlarges the boundary of the Rect so that it contains the coordinate. The special case occurring
	 * at longitude +180° and -180° is not considered.
	 * 
	 * @param gc
	...
	/**
	 * Enlarges the boundary of the Rect so that it contains the rectangle. The special case occurring
	 * at longitude +180° and -180° is not considered.
	 * 
	 * @param otherRect
	 ...
	 */
	public boolean includes(GeoCoordinate coordinate) {
		return includes(coordinate.getLatitudeE6(), coordinate.getLongitudeE6());
	}
	...
	public boolean overlaps(Rect rect) {
		return overlaps(
                this.minLongitudeE6, this.maxLongitudeE6, this.minLatitudeE6, this.maxLatitudeE6,
                rect.minLongitudeE6, rect.maxLongitudeE6, rect.minLatitudeE6, rect.maxLatitudeE6);
	}
