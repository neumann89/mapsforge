...
public class KCenterClusteringAlgorithm {
	...
	private static void expandClusters(Graph graph, KCenterClustering clustering) {
		...
		for (KCenterClustering.KCenterCluster c : clustering.getClusters()) {
	...
	private static void sampleDown(Graph graph, KCenterClustering clustering, int k,
		...
		while (clustering.size() > k) {
			KCenterClustering.KCenterCluster cluster = chooseClusterForRemoval(graph, clustering, heuristik);
	...
	private static void removeClusterAndRearrange(Graph graph, KCenterClustering clustering,
			KCenterClustering.KCenterCluster cluster) {
		// remove the cluster
		KCenterClustering.KCenterCluster[] adjClusters = getAdjacentClusters(graph, clustering, cluster);
		...
		// initialize heap and enqueue centers
		BinaryMinHeap<HeapItem, Integer> queue = new BinaryMinHeap<HeapItem, Integer>(10000);
		for (KCenterClustering.KCenterCluster c : adjClusters) {
			HeapItem item = new HeapItem(c.getCenterVertex(), 0, c.getCenterVertex());
			queue.insert(item);
			enqueuedVertices.put(c.getCenterVertex(), item);
		}
	 ...
	 */
	private static KCenterClustering.KCenterCluster[] getAdjacentClusters(Graph graph,
			KCenterClustering clustering, KCenterClustering.KCenterCluster cluster) {
		THashSet<KCenterClustering.KCenterCluster> set = new THashSet<KCenterClustering.KCenterCluster>();
		for (int v : cluster.getVertices()) {
			for (Edge e : graph.getVertex(v).getOutboundEdges()) {
				KCenterClustering.KCenterCluster c = clustering.getCluster(e.getTarget().getId());
				if (c != null && !c.equals(cluster)) {
					set.add(c);
				}
			}
		}
		KCenterClustering.KCenterCluster[] adjClusters = new KCenterClustering.KCenterCluster[set.size()];
	 ...
	 */
	private static KCenterClustering.KCenterCluster chooseClusterForRemoval(Graph graph,
			KCenterClustering clustering, int heuristik) {
		switch (heuristik) {
			case HEURISTIC_MIN_RADIUS:
				return getMinCluster(clustering, new Comparator<KCenterClustering.KCenterCluster>() {

					@Override
					public int compare(KCenterClustering.KCenterCluster c1, KCenterClustering.KCenterCluster c2) {
	 ...
	 */
	private static KCenterClustering.KCenterCluster getMinCluster(KCenterClustering clustering,
			Comparator<KCenterClustering.KCenterCluster> comp) {
		KCenterClustering.KCenterCluster min = clustering.getClusters().iterator().next();
		for (KCenterClustering.KCenterCluster c : clustering.getClusters()) {
	...
	private static int clusterUnconnectedComponents(Graph graph, KCenterClustering clustering) {
		...
		for (Iterator<? extends Vertex> iter = graph.getVertices(); iter.hasNext();) {
			...
			if (clustering.getCluster(v) == null) {
				...
				KCenterClustering.KCenterCluster cluster = clustering.addCluster(v);
				numCreatedClusters++;
