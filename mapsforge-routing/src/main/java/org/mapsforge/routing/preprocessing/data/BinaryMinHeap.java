...
public class BinaryMinHeap<I extends IBinaryHeapItem<K>, K extends Comparable<K>> {
    ...
    /**
     * Returns the priority item without removing it.
     *
     * @return The priority item.
     */
    public I peek() {
    ...
    /**
     * Changes the key of the item to the new value.
     *
     * @param item      The item, which has to be updated.
     * @param newKey    The new key for the item.
     */
    public void changeKey(I item, K newKey) {
        int cmp = item == null || newKey == null ? 0 : item.getHeapKey().compareTo(newKey);
        if (cmp != 0) {
            item.setHeapKey(newKey);
            if (cmp == 1) {// new key is lower than the old one
                moveUpward(item.getHeapIndex());
            }
            else {// cmp == -1; new key is greater than the old one
                minHeapify(item);
            }
        }
    }
