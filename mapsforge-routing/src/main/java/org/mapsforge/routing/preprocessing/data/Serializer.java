...
class Serializer {
	...
	private final static byte[] BITMASK_BYTE_HIGH_CLEARED = new byte[] { (byte) 0xff, 0x7f,
			0x3f, 0x1f, 0x0f, 0x07, 0x03, 0x01, 0x00 };

	private final static byte[] BITMASK_BYTE_LOW_CLEARED = new byte[] { (byte) 0xff,
	...
	private static void writeBits(byte val, int nBits, byte[] buff, int byteOffset,
		...
		if (_nBits + _bitOffset >= BITS_PER_BYTE) {
			buff[_byteOffset] &= BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE - _bitOffset];
		...
		if (_nBits > 0) {
			buff[_byteOffset] = (byte) ((buff[_byteOffset] & BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE
					- _bitOffset]) | (buff[_byteOffset] & BITMASK_BYTE_LOW_CLEARED[_bitOffset
					+ _nBits]));
			buff[_byteOffset] |= shl((byte) (_val & BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE
					- _nBits]), _bitOffset);
		}
	}
	...
	public static void writeByte(byte val, byte[] buff, int byteOffset, int bitOffset) {
		buff[byteOffset] = (byte) ((buff[byteOffset] & BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE
				- bitOffset]) | shl(val, bitOffset));
		buff[byteOffset + 1] = (byte) ((buff[byteOffset + 1] &
				BITMASK_BYTE_LOW_CLEARED[bitOffset]) | shr(
	...
	private static byte readBits(byte[] buff, int nBits, int byteOffset, int bitOffset) {
		if (bitOffset + nBits > BITS_PER_BYTE) {
			return (byte) (readByte(buff, byteOffset, bitOffset) & BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE
					- nBits]);
		}
		return (byte) (shr(buff[byteOffset], bitOffset) & BITMASK_BYTE_HIGH_CLEARED[BITS_PER_BYTE
				- nBits]);
	}
	...
	public static long readUInt(byte[] buff, int nBits, int byteOffset, int bitOffset) {
		...
		if (_nBits > 0) {
			BUFFER[j++] = readBits(buff, _nBits, _byteOffset, bitOffset);
		}
		while (j < 8) {
			BUFFER[j++] = 0;
		}
