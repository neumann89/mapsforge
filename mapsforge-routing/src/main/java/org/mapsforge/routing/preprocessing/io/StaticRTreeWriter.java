...
/**
 * This class implements a static r-tree for indexing two dimensional rectangles. Packing is done using
 * the 'SortTileRecursive' (STR) algorithm which gives an optimal space utilization and local
 * clustering. Data can be retrieved by an overlaps query against a given rectangle.
 */
public class StaticRTreeWriter {
	...
	/**
	 * SortTileRecursive(STR) Algorithm for static r-tree packing. All given arrays must be of equal
	 * length.
	 * 
	 * @param minLon
	...
	public static void packSortTileRecursive(final int[] minLon, final int[] maxLon,
			...
			File targetFile, final byte[] headerMagic) throws IOException {
		IndexSortableNodeEntries rectangles = new IndexSortableNodeEntries(minLon, maxLon,
				minLat, maxLat, pointer);
		RandomAccessFile raf = new RandomAccessFile(targetFile, "rw");
		packSTR(rectangles, blockSizeBytes, raf, 1, true, headerMagic);
	...
	private static void packSTR(IndexSortableNodeEntries rectangles, int blockSizeBytes,
			RandomAccessFile raf, int nodeId, boolean isLeaf, final byte[] headerMagic)
			throws IOException {
		int _nodeId = nodeId;
		...
		for (int i = 0; i < rectangles.size(); i++) {
			...
			if (i % (s * b) == 0) {
				...
				quicksort.sort(rectangles, i, sliceEnd);
			}
			...
			if (i % b == 0) {
				...
				int nodeEnd = i + Math.min(b, rectangles.size() - i);

				raf.seek(_nodeId * blockSizeBytes);
				raf.writeBoolean(isLeaf);
				raf.writeShort(nodeEnd - i);
		...
		if (n > 1) {
			packSTR(parentEntries, blockSizeBytes, raf, _nodeId, false, headerMagic);
		} else {
			// write header
			raf.seek(0);
			raf.write(headerMagic);
			raf.writeInt(blockSizeBytes);
		}
	}

	/**
	 * The QuickSort algorithm used requires to implement the IndexedSortable interface. This class
	 * exists only for this purpose, and is used during recursive tree construction for holding all
	 * entries of the current tree level.
	 */
	private static class IndexSortableNodeEntries implements IndexedSortable {
		...
		/**
		 * Set to true for sorting by comparing the longitude. Set to false for sorting by comparing
		 * latitude.
		 * 
		 * @param b
		 *            switch.
