 ...
 */
package org.mapsforge.routing.android.data;
...
/**
 * This class implements a static rtree variant. It supports the overlaps query which is the key to
 * efficient nearest neighbor queries limited to a specific range.
 */
public final class StaticRTree implements Closeable {
	/**
	 ...
	 * @param headerMagic
	 *            The header magic bytes written at the start of the header of this R-tree. This will be
	 *            used for checking, if the file is likely contain a valid R-tree representation.
	 * @throws IOException
	 *             if there was an error reading file, or the given location does not contain a valid
	 *             r-tree.
	 */
	public StaticRTree(File file, long startAddr, long endAddr, byte[] headerMagic) throws IOException {
		this.startAddr = startAddr;
		this.raf = new RandomAccessFile(file, "r");

		// read header
		raf.seek(startAddr);
		byte[] fileHeaderMagic = new byte[headerMagic.length];
		raf.readFully(fileHeaderMagic);
		if (!Arrays.equals(fileHeaderMagic, headerMagic)) {
	...
	/**
	 * Get integers values associated with the indexed rectangles overlapping the given rectangle.
	 * 
	 * @param r
	...
	private static class RtreeNode {
		 ...
		 * satellite data. If this node is a inner node, the pointer points to the x-th node of the
		 * file.
		 */
		final int[] pointer;
